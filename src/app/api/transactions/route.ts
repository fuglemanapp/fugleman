...
export async function GET(request: Request) {
  ...
  return withUserDb(user.id, async (database) => {
    const context = await resolveFinancialContext(user.id, searchParams.get("context"), database);
    ...
    const [transactions, installments] = await Promise.all([
      ...
      database.cardInstallment.findMany({
...
export async function POST(request: Request) {
  ...
  return withUserDb(user.id, async (database) => {
    const context = await resolveFinancialContext(user.id, typeof payload.context === "string" ? payload.context : null, database);
    if (!context) return NextResponse.json({ error: "Espaço financeiro inválido ou sem acesso." }, { status: 403 });
    const suggestion = await applyTransactionRule(user.id, description, { category, type }, database);
    const transaction = await database.transaction.create({ data: { description, category: suggestion.category, type: suggestion.type || type, amount, date, userId: user.id, teamId: context.teamId }, include: { user: { select: { id: true, name: true, email: true } } } });
...
export async function DELETE(request: Request) {
  ...
  return withUserDb(user.id, async (database) => {
    ...
    if (!context) return NextResponse.json({ error: "Espaço financeiro inválido ou sem acesso." }, { status: 403 });
    const transaction = await database.transaction.findFirst({ where: { id, ...transactionContextWhere(context) }, select: { id: true, source: true } });
    ...
    await database.transaction.delete({ where: { id: transaction.id } });
