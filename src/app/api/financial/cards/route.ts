...
export async function GET(request: Request) {
  ...
  return withUserDb(user.id, async (database) => {
    const context = await resolveFinancialContext(user.id, new URL(request.url).searchParams.get("context"), database);
    ...
    const cards = await database.creditCard.findMany({ where, include: { user: { select: { id: true, name: true, email: true } } }, orderBy: [{ isActive: "desc" }, { createdAt: "asc" }] });
...
export async function POST(request: Request) {
  ...
  return withUserDb(user.id, async (database) => {
    const context = await resolveFinancialContext(user.id, typeof body?.context === "string" ? body.context : null, database);
    if (!context) return NextResponse.json({ error: "Espaço financeiro inválido ou sem acesso." }, { status: 403 });
    const card = await database.creditCard.create({ data: { userId: user.id, teamId: context.type === "FAMILY" ? context.teamId : null, name, issuer, lastFour, color, limit, closingDay, dueDay } , include: { user: { select: { id: true, name: true, email: true } } } });
...
export async function PATCH(request: Request) {
  ...
  return withUserDb(user.id, async (database) => {
    const context = await resolveFinancialContext(user.id, typeof body?.context === "string" ? body.context : null, database);
    if (!context) return NextResponse.json({ error: "Espaço financeiro inválido ou sem acesso." }, { status: 403 });
    const card = await database.creditCard.findFirst({ where: context.type === "FAMILY" ? { id, userId: user.id, teamId: context.teamId } : { id, userId: user.id, teamId: null } });
    ...
    const updated = await database.creditCard.update({ where: { id: card.id }, data: { name, issuer, lastFour, color, limit, closingDay, dueDay, isActive: typeof body?.isActive === "boolean" ? body.isActive : card.isActive }, include: { user: { select: { id: true, name: true, email: true } } } });
...
export async function DELETE(request: Request) {
  ...
  return withUserDb(user.id, async (database) => {
    const context = await resolveFinancialContext(user.id, params.get("context"), database);
    if (!context) return NextResponse.json({ error: "Espaço financeiro inválido ou sem acesso." }, { status: 403 });
    const card = await database.creditCard.findFirst({ where: context.type === "FAMILY" ? { id, userId: user.id, teamId: context.teamId } : { id, userId: user.id, teamId: null }, select: { id: true } });
    if (!card) return NextResponse.json({ error: "Cartão não encontrado ou sem permissão para remover." }, { status: 404 });
    await database.creditCard.update({ where: { id: card.id }, data: { isActive: false } });
