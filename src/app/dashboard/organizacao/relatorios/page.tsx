...
export default function RelatoriosOrgPage() {
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [selectedReport, setSelectedReport] = useState<ReportConfig | null>(null)

  const relatorios = [
    { title: "Por Projeto", desc: "Volume de tarefas nos projetos", icon: <Layers className="w-5 h-5" />, color: "bg-purple-100 text-purple-600" },
    { title: "Por Prioridade", desc: "Distribuição de tarefas entre Normal, Alta e Baixa", icon: <Layers className="w-5 h-5" />, color: "bg-blue-100 text-blue-500" }, // Usando Layers genérico como no print, ideal seria empilhado
    { title: "Por Status", desc: "Atrasadas, Pendentes vs Concluídas", icon: <CheckCircle2 className="w-5 h-5" />, color: "bg-emerald-100 text-emerald-500" },
    { title: "Por Categoria", desc: "Volume de alocação por TAGs", icon: <Tag className="w-5 h-5" />, color: "bg-orange-100 text-orange-500" },
    { title: "Lead Time & Ciclo", desc: "Tempo médio de resolução diária", icon: <Clock className="w-5 h-5" />, color: "bg-pink-100 text-pink-500" },
  ];

  const handleCardClick = (report: ReportConfig) => {
    ...
    <div className="flex flex-col min-h-screen bg-transparent transition-colors duration-300 dark:bg-[#0B0F19] font-sans text-slate-900">
      ...
      <header className="bg-white dark:bg-[#151521] border-b border-slate-200 dark:border-slate-800 h-14 flex items-center px-4 md:px-6 justify-between shrink-0 sticky top-0 z-50">
        <div className="flex items-center gap-12">
          ...
          {/* Primary Nav (Simulada para visualização, será substituída pelo script global se necessário) */}
      ...
      <main className="flex-1 p-4 md:p-8 overflow-auto flex flex-col items-center">
        ...
        <div className="w-full max-w-5xl bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 mb-6 flex flex-col">
          <h1 className="text-xl md:text-2xl font-extrabold text-slate-900 mb-1">Relatórios Organizacionais</h1>
          <p className="text-sm text-slate-500">Monitore o desempenho da sua equipe, eficiência de processos e progresso dos projetos.</p>
        </div>
        ...
        <div className="w-full max-w-5xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {relatorios.map((rel, index) => (
            <div 
              key={index}
              onClick={() => handleCardClick({ title: `Tarefas ${rel.title}`, icon: rel.icon })}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-[500px] overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
            ...
            <div className="p-8 pb-6 flex items-center gap-4 relative">
              ...
              <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 bg-indigo-50 text-indigo-500">
                <PieChart className="w-5 h-5" />
              </div>
              <div>
                <h2 className="text-xl font-extrabold text-slate-900 leading-tight">{selectedReport.title}</h2>
                <p className="text-[13px] text-slate-500 mt-1">Configure os filtros do documento</p>
              </div>
            </div>
            ...
            <div className="px-8 pb-8 flex flex-col gap-6">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-bold text-slate-500 tracking-wider uppercase">FILTROS TEMPORAIS</span>
              </div>

              <div className="flex flex-col gap-5">
                <div>
                  <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">DATA INICIAL</label>
                  <div className="flex items-center justify-between border border-slate-200 rounded-xl px-4 py-3 bg-white focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-500 transition-all">
                    <input
                      type="text"
