    ...
    <div className="flex flex-col min-h-screen bg-transparent transition-colors duration-300 dark:bg-[#0B0F19] font-sans text-slate-900">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-auto flex flex-col items-center">
        ...
        <div className="w-full max-w-5xl bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl md:text-2xl font-extrabold text-slate-900 mb-1">Meus Arquivos</h1>
            <p className="text-sm text-slate-500">Armazene, organize e encontre seus documentos com IA.</p>
          </div>
          <button
        ...
        <div className="w-full max-w-5xl bg-white rounded-2xl shadow-sm border border-slate-100 p-4 mb-6 flex flex-col md:flex-row items-center justify-between gap-4">
          {/* Busca */}
          <div className="flex items-center border border-slate-200 rounded-xl px-4 py-2.5 bg-white flex-1 max-w-md focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-500 transition-all">
            <Search className="w-4 h-4 text-slate-400 shrink-0 mr-3" />
            <input type="text" placeholder="Buscar arquivos..." className="w-full text-slate-700 text-sm font-medium focus:outline-none bg-transparent placeholder:text-slate-400" />
          </div>

          {/* Ações */}
          <div className="flex items-center gap-3">
            {/* Grid/List Toggle */}
            <div className="flex bg-slate-50 p-1 rounded-xl border border-slate-100 text-sm">
              <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500'}`}>
                <Grid3X3 className="w-4 h-4" />
              </button>
              <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500'}`}>
                <List className="w-4 h-4" />
              </button>
            </div>

            <button onClick={() => setShowShare(true)} className="px-4 py-2 text-slate-500 hover:text-slate-800 hover:bg-slate-50 rounded-xl text-xs font-medium flex items-center gap-2 border border-slate-200 transition-colors">
              <Share2 className="w-3.5 h-3.5" /> Compartilhar
            </button>
            <button onClick={() => setShowHistory(true)} className="px-4 py-2 text-slate-500 hover:text-slate-800 hover:bg-slate-50 rounded-xl text-xs font-medium flex items-center gap-2 border border-slate-200 transition-colors">
              <History className="w-3.5 h-3.5" /> Histórico
            </button>
            <button onClick={() => setShowNewFolder(true)} className="px-4 py-2 text-slate-500 hover:text-slate-800 hover:bg-slate-50 rounded-xl text-xs font-medium flex items-center gap-2 border border-slate-200 transition-colors">
              <Folder className="w-3.5 h-3.5" /> Nova Pasta
            </button>
            <button onClick={() => setShowUpload(true)} className="px-4 py-2 bg-[#F3E8FF] hover:bg-[#E9D5FF] text-[#9333EA] font-semibold text-xs rounded-xl transition-colors flex items-center gap-2">
        ...
        {/* Empty State */}
        <div className="w-full max-w-5xl flex flex-col items-center justify-center py-24 text-center">
          <div className="relative w-24 h-24 bg-gradient-to-br from-emerald-100/50 to-teal-100/50 rounded-full flex items-center justify-center mb-8 shadow-sm">
            <div className="absolute inset-0 bg-white/40 rounded-full backdrop-blur-sm"></div>
            <FolderOpen className="w-10 h-10 text-emerald-500 relative z-10" fill="currentColor" fillOpacity="0.2" strokeWidth={1.5} />
          </div>
          <h2 className="text-2xl font-extrabold text-slate-900 mb-3 tracking-tight">Nenhum arquivo encontrado</h2>
          <p className="text-[15px] text-slate-500 max-w-[320px] mx-auto mb-8 leading-relaxed">
            Envie documentos pelo WhatsApp ou faça upload diretamente pelo painel.
          </p>
          <button onClick={() => setShowUpload(true)} className="px-6 py-2.5 bg-transparent border border-slate-200 hover:bg-slate-50 text-slate-600 font-medium text-sm rounded-xl transition-colors">
            Fazer Upload
          </button>
        </div>
      </main>
      ...
      {showUpload && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-[500px] flex flex-col">
            <div className="p-8 pb-6 flex items-center gap-4 relative border-b border-slate-100">
              <button onClick={() => setShowUpload(false)} className="absolute right-6 top-6 w-8 h-8 bg-slate-100 hover:bg-slate-200 text-slate-500 rounded-full flex items-center justify-center transition-colors"><X className="w-4 h-4" /></button>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-emerald-50 text-emerald-500 border border-emerald-100"><Upload className="w-6 h-6" /></div>
              <div>
                <h2 className="text-xl font-extrabold text-slate-900">Fazer Upload</h2>
                <p className="text-[13px] text-slate-500 mt-1">Envie documentos, imagens ou arquivos.</p>
              </div>
            </div>
            <div className="px-8 py-8 flex flex-col gap-6">
              {/* Dropzone */}
              <div className="border-2 border-dashed border-slate-200 rounded-2xl p-10 flex flex-col items-center justify-center text-center hover:border-purple-300 hover:bg-purple-50/30 transition-all cursor-pointer group">
                <Upload className="w-10 h-10 text-slate-300 mb-4 group-hover:text-purple-400 transition-colors" />
                <p className="font-bold text-slate-700 mb-1">Arraste arquivos aqui</p>
                <p className="text-xs text-slate-400">ou clique para selecionar do computador</p>
                <p className="text-[10px] text-slate-300 mt-4">PDF, DOCX, XLSX, JPG, PNG (máx. 25MB)</p>
              </div>
            </div>
            <div className="px-8 pb-8 flex justify-end">
              <button onClick={() => setShowUpload(false)} className="px-8 py-3 bg-[#F8F1FF] hover:bg-[#F3E8FF] text-[#9333EA] font-semibold text-sm rounded-xl transition-colors shadow-sm flex items-center gap-2">
                <Check className="w-4 h-4" /> Enviar Arquivos
              </button>
            </div>
          </div>
        </div>
      ...
      {showShare && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-[500px] flex flex-col">
            <div className="p-8 pb-6 flex items-center gap-4 relative border-b border-slate-100">
              <button onClick={() => setShowShare(false)} className="absolute right-6 top-6 w-8 h-8 bg-slate-100 hover:bg-slate-200 text-slate-500 rounded-full flex items-center justify-center transition-colors"><X className="w-4 h-4" /></button>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-blue-50 text-blue-500 border border-blue-100"><Share2 className="w-6 h-6" /></div>
              <div>
                <h2 className="text-xl font-extrabold text-slate-900">Compartilhar Pasta</h2>
                <p className="text-[13px] text-slate-500 mt-1">Gere um link público para compartilhar.</p>
              </div>
            </div>
            <div className="px-8 py-8 flex flex-col gap-6">
              <div>
                <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">LINK DE COMPARTILHAMENTO</label>
                <div className="flex items-center border border-slate-200 rounded-xl px-4 py-3 bg-white focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-500 transition-all">
                  <Link2 className="w-4 h-4 text-slate-400 shrink-0 mr-3" />
                  <input type="text" readOnly value="https://app.fugleman.com/drive/share/xK9mZ2..." className="w-full text-slate-600 font-medium focus:outline-none bg-transparent text-sm" />
                  <button className="text-slate-400 hover:text-slate-600 transition-colors ml-2"><Copy className="w-4 h-4" /></button>
                </div>
              </div>
              <div>
                <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">EXPIRAÇÃO</label>
                <div className="relative">
                  <select defaultValue="7" className="w-full appearance-none border border-slate-200 rounded-xl px-4 py-3 text-slate-800 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all bg-white cursor-pointer">
            ...
            <div className="px-8 pb-8 flex justify-end gap-3">
              <button onClick={() => setShowShare(false)} className="px-6 py-2.5 bg-transparent border border-slate-200 hover:bg-slate-50 text-slate-600 font-medium text-sm rounded-xl transition-colors">Cancelar</button>
              <button onClick={() => setShowShare(false)} className="px-6 py-2.5 bg-[#F8F1FF] hover:bg-[#F3E8FF] text-[#9333EA] font-semibold text-sm rounded-xl transition-colors shadow-sm flex items-center gap-2">
      ...
      {/* ========== MODAL 3: HISTÓRICO (Slide-over da Direita) ========== */}
      {showHistory && (
        <div className="fixed inset-0 z-[100] flex justify-end bg-black/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-[420px] h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            <div className="p-6 flex items-center justify-between border-b border-slate-100">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-orange-50 text-orange-500 border border-orange-100"><History className="w-5 h-5" /></div>
                <div>
                  <h2 className="text-lg font-extrabold text-slate-900">Histórico</h2>
                  <p className="text-[11px] text-slate-500">Atividade recente nos seus arquivos.</p>
                </div>
              </div>
              <button onClick={() => setShowHistory(false)} className="w-8 h-8 bg-slate-100 hover:bg-slate-200 text-slate-500 rounded-full flex items-center justify-center transition-colors"><X className="w-4 h-4" /></button>
            </div>
            <div className="flex-1 p-6 overflow-auto flex flex-col items-center justify-center text-center">
              ...
              <h3 className="font-bold text-slate-700 mb-2">Nenhuma atividade recente</h3>
              <p className="text-xs text-slate-400 max-w-xs">As ações de upload, exclusão e compartilhamento aparecerão aqui.</p>
            </div>
          </div>
        </div>
      )}
      ...
      {showNewFolder && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-[450px] flex flex-col">
            <div className="p-8 pb-6 flex items-center gap-4 relative border-b border-slate-100">
              <button onClick={() => setShowNewFolder(false)} className="absolute right-6 top-6 w-8 h-8 bg-slate-100 hover:bg-slate-200 text-slate-500 rounded-full flex items-center justify-center transition-colors"><X className="w-4 h-4" /></button>
              <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-amber-50 text-amber-500 border border-amber-100"><Folder className="w-6 h-6" /></div>
              <div>
                <h2 className="text-xl font-extrabold text-slate-900">Nova Pasta</h2>
                <p className="text-[13px] text-slate-500 mt-1">Crie uma pasta para organizar seus arquivos.</p>
              </div>
            </div>
            <div className="px-8 py-8 flex flex-col gap-6">
              <div>
                <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">NOME DA PASTA</label>
                <input type="text" placeholder="Ex: Documentos Pessoais" className="w-full border border-slate-200 rounded-xl px-4 py-3.5 text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all bg-white" />
              </div>
            </div>
            <div className="px-8 pb-8 flex justify-end">
              <button onClick={() => setShowNewFolder(false)} className="px-8 py-3 bg-[#F8F1FF] hover:bg-[#F3E8FF] text-[#9333EA] font-semibold text-sm rounded-xl transition-colors shadow-sm flex items-center gap-2">
                <Check className="w-4 h-4" /> Criar Pasta
              </button>
            </div>
          </div>
        </div>
