    ...
    <div className="flex flex-col min-h-screen bg-transparent transition-colors duration-300 dark:bg-[#0B0F19] font-sans text-slate-900">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-auto flex flex-col items-center">
        ...
        <div className="w-full max-w-5xl bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl md:text-2xl font-extrabold text-slate-900 mb-1">Pessoas</h1>
            <p className="text-sm text-slate-500">Gerencie seus clientes, fornecedores e parceiros.</p>
          </div>
          <button
        ...
        {/* Empty State */}
        <div className="w-full max-w-5xl flex flex-col items-center justify-center py-32 text-center">
          <div className="w-16 h-16 bg-slate-100 text-slate-400 rounded-full flex items-center justify-center mb-6 shadow-sm border border-slate-200">
            <Users className="w-8 h-8" strokeWidth={1.5} />
          </div>
          <h2 className="text-base font-medium text-slate-500 mb-2">Nenhuma pessoa encontrada.</h2>
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-[800px] overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
            ...
            <div className="p-8 pb-6 flex items-center gap-4 relative border-b border-slate-100">
              ...
              <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 bg-purple-50 text-purple-500 border border-purple-100">
                <UserPlus className="w-6 h-6" />
              </div>
              <div>
                <h2 className="text-xl font-extrabold text-slate-900 leading-tight">Nova Pessoa</h2>
                <p className="text-[13px] text-slate-500 mt-1">Cadastre um novo cliente ou fornecedor</p>
              </div>
            </div>

            {/* Modal Body */}
            <div className="px-8 py-8 flex flex-col md:flex-row gap-12 bg-white">
              ...
              <div className="flex-1 flex flex-col gap-6">
                ...
                <div className="flex bg-slate-50 p-1.5 rounded-2xl border border-slate-100">
                  <button
                    ...
                    className={`flex-1 py-3 px-4 rounded-xl flex items-center justify-center gap-2 text-sm font-semibold transition-all ${personType === 'fisica' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
                  >
                    <User className="w-4 h-4" /> Física
                  </button>
                  <button
                    onClick={() => setPersonType('juridica')}
                    className={`flex-1 py-3 px-4 rounded-xl flex items-center justify-center gap-2 text-sm font-semibold transition-all ${personType === 'juridica' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
                  >
                    <Building2 className="w-4 h-4" /> Jurídica
                  </button>
                </div>

                {/* Campos Dinâmicos (Mudam conforme Física/Jurídica) */}
                {personType === 'fisica' ? (
                  <>
                    <div>
                      <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">NOME COMPLETO</label>
                      <input
                        type="text"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">CPF</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">PAPEL</label>
                        <div className="relative">
                          <select defaultValue="Ambos" className="w-full appearance-none border border-slate-200 rounded-xl px-4 py-3.5 text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all bg-white cursor-pointer">
                            <option value="Ambos">Ambos</option>
                            <option value="Pagador (Cliente)">Pagador (Cliente)</option>
                            <option value="Recebedor (Fornecedor)">Recebedor (Fornecedor)</option>
                            <option value="Contato (Agenda)">Contato (Agenda)</option>
                            <option value="Fugleman (Financeiro)">Fugleman (Financeiro)</option>
                          </select>
                  ...
                  <>
                    <div>
                      <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">RAZÃO SOCIAL</label>
                      <input
                        type="text"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">CNPJ</label>
                        <input
                          type="text"
              ...
              <div className="flex-1 flex flex-col gap-6">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] font-bold text-slate-500 tracking-wider uppercase">CONTATOS</span>
                </div>

                <div>
                  <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">E-MAIL <span className="text-slate-400 font-normal">(OPCIONAL)</span></label>
                  <input
                    type="email"
                    placeholder="email@exemplo.com"
                    className="w-full border border-slate-200 rounded-xl px-4 py-3.5 text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all bg-white placeholder:text-slate-400"
                  />
                </div>

                <div>
                  <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">TELEFONE <span className="text-slate-400 font-normal">(OPCIONAL)</span></label>
                  <input
                    type="tel"
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">WHATSAPP <span className="text-slate-400 font-normal">(OPCIONAL)</span></label>
                  <input
                    type="tel"
            ...
            <div className="px-8 pb-8 flex justify-end">
              <button
                onClick={() => setIsModalOpen(false)}
                className="px-8 py-3 bg-[#F8F1FF] hover:bg-[#F3E8FF] text-[#9333EA] font-semibold text-sm rounded-xl transition-colors shadow-sm flex items-center gap-2"
              >
                <Check className="w-4 h-4" /> Salvar Pessoa
              </button>
            </div>
