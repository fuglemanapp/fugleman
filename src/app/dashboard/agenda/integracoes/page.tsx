...
export default function IntegracoesAgendaPage() {
    ...
    <div className="flex flex-col min-h-screen bg-transparent transition-colors duration-300 dark:bg-[#0B0F19] font-sans text-slate-900">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-auto flex flex-col items-center">
        ...
        <div className="w-full max-w-5xl bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 mb-6 flex flex-col">
          <h1 className="text-xl md:text-2xl font-extrabold text-slate-900 mb-1">Integrações de Agenda</h1>
          <p className="text-sm text-slate-500">Gerencie suas conexões com o Google Agenda e sincronize seus compromissos.</p>
        </div>
        ...
        <div className="w-full max-w-5xl flex flex-col gap-6">
          ...
          {/* Card Google Agenda */}
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:shadow-md transition-shadow">
            
            {/* Esquerda: Info */}
            <div className="flex flex-col gap-3 max-w-2xl">
              <div className="flex items-center gap-4">
                {/* Ícone fake (Google Calendar) */}
                <div className="w-8 h-8 rounded-lg bg-white shadow-sm border border-slate-100 flex items-center justify-center font-bold text-blue-500 text-xs">
                  31
                </div>
                <h3 className="text-[17px] font-extrabold text-slate-900 flex items-center gap-3">
                  Google Agenda
              ...
              <p className="text-[13px] text-slate-500 leading-relaxed">
                Sincronize automaticamente seus compromissos entre o sistema e sua agenda do Google. Permite importação (Google → Sistema) e exportação (Sistema → Google).
              </p>
            </div>

            {/* Direita: Toggle */}
            <div className="flex items-center gap-4 shrink-0">
              <span className="text-sm font-semibold text-slate-700">Habilitar Integração</span>
              <div
                onClick={() => setGoogleEnabled(!googleEnabled)}
                className={`w-12 h-6 rounded-full cursor-pointer relative transition-colors duration-300 ${googleEnabled ? 'bg-[#00C853]' : 'bg-slate-200'}`}
              >
                <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow-sm transform transition-transform duration-300 ${googleEnabled ? 'translate-x-7' : 'translate-x-1'}`}></div>
              </div>
            </div>
