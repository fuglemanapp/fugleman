    ...
    <div className="flex flex-col min-h-screen bg-transparent transition-colors duration-300 dark:bg-[#0B0F19] font-sans text-slate-900">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-auto flex flex-col items-center">
        ...
        <div className="w-full max-w-5xl bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl md:text-2xl font-extrabold text-slate-900 mb-1">Meus Cartões</h1>
            <p className="text-sm text-slate-500">Gerencie seus cartões de crédito e acompanhe seus gastos.</p>
          </div>
          <button
        ...
        <div className="w-full max-w-5xl flex flex-col items-center justify-center py-20 text-center">
          <div className="relative w-24 h-24 bg-gradient-to-br from-pink-100/50 to-purple-100/50 rounded-full flex items-center justify-center mb-8 shadow-sm">
            ...
            <CreditCard className="w-10 h-10 text-pink-500 relative z-10" fill="currentColor" strokeWidth={1.5} />
          </div>
          <h2 className="text-2xl font-extrabold text-slate-900 mb-3 tracking-tight">Nenhum cartão cadastrado</h2>
          <p className="text-[15px] text-slate-500 max-w-[320px] mx-auto mb-8 leading-relaxed">
            Adicione seus cartões de crédito para gerenciar limites e faturas.
          </p>
          <button
            onClick={() => setIsModalOpen(true)}
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-[700px] overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
            ...
            <div className="p-8 pb-6 flex flex-col relative border-b border-slate-100">
              ...
              <div className="flex items-center gap-4 mb-2">
                ...
                <div>
                  <h2 className="text-2xl font-extrabold text-slate-900 leading-tight">Novo Cartão</h2>
                  <p className="text-sm text-slate-500 mt-1">Escolha como deseja cadastrar este cartão.</p>
                </div>
              </div>
            </div>
            ...
            {modalStep === 'choice' ? (
              <div className="p-8 bg-[#FAFAFA] grid md:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm hover:border-purple-300 hover:shadow-md transition-all flex flex-col h-full group cursor-pointer">
                  ...
                  <h3 className="text-lg font-bold text-slate-900 mb-3">Cartão OpenFinance</h3>
                  <p className="text-[13px] text-slate-500 leading-relaxed mb-8 flex-1">
                    Conecte seu banco via Open Finance para importar cartões, limites, faturas e movimentações de forma automática e segura.
                  </p>
                ...
                >
                  ...
                  <h3 className="text-lg font-bold text-slate-900 mb-3">Cartão Manual</h3>
                  <p className="text-[13px] text-slate-500 leading-relaxed mb-8 flex-1">
                    Cadastre limite, fechamento e vencimento manualmente. Ideal para controlar cartões sem integração ou quando você quer lançar tudo por conta própria.
                  </p>
                  <button className="text-sm font-bold text-blue-600 flex items-center gap-2 group-hover:text-blue-700 transition-colors">
                    Preencher dados do cartão <ArrowRightLeft className="w-4 h-4 ml-1" />
                  </button>
                </div>
              </div>
            ) : (
              <div className="p-8 bg-[#FAFAFA] flex flex-col gap-8">
                <div className="grid md:grid-cols-12 gap-8">
                  {/* Esquerda: Dados do Cartão (Maior) */}
                  <div className="md:col-span-7 flex flex-col gap-6">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-blue-500"></div>
                      <span className="text-[10px] font-bold text-slate-500 tracking-wider uppercase">DADOS DO CARTÃO</span>
                    </div>

                    <div>
                      <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">Nome do Cartão</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">Limite do Cartão (R$)</label>
                      <div className="flex items-center border border-slate-100 rounded-xl px-4 py-3.5 bg-slate-50 focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-500 transition-all focus-within:bg-white">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">Dia de Fechamento</label>
                        <input
                          type="text"
                          placeholder="Dia (1-31)"
                          className="w-full border border-slate-100 rounded-xl px-4 py-3.5 text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-slate-50 focus:bg-white"
                        />
                      </div>
                      <div>
                        <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">Dia de Vencimento</label>
                        <input
                          type="text"
                          placeholder="Dia (1-31)"
                          className="w-full border border-slate-100 rounded-xl px-4 py-3.5 text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-slate-50 focus:bg-white"
                  ...
                  <div className="md:col-span-5 flex flex-col gap-6">
                    ...
                    <div className="border border-slate-100 rounded-2xl p-5 flex items-center justify-between bg-white shadow-sm hover:border-slate-200 transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="font-bold text-slate-900 text-[13px]">Cartão Principal</h4>
                          <p className="text-[10px] text-slate-400">Definir como padrão</p>
                        </div>
                      </div>
                      <div
                        onClick={() => setIsMainCard(!isMainCard)}
                        className={`w-10 h-5 rounded-full cursor-pointer relative transition-colors duration-300 ${isMainCard ? 'bg-blue-500' : 'bg-slate-200'}`}
                      >
                        <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-sm transform transition-transform duration-300 ${isMainCard ? 'translate-x-5.5' : 'translate-x-0.5'}`}></div>
                      </div>
                    </div>
                  </div>
                </div>
