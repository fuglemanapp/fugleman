...
export default function RelatoriosPage() {
  const [selectedReport, setSelectedReport] = useState<{title: string, icon: JSX.Element, color: string} | null>(null)

  const relatorios = [
    { title: "Por Categoria", desc: "Gastos divididos por categoria", icon: <PieChart className="w-5 h-5" />, color: "bg-orange-100 text-orange-500" },
    { title: "Por Usuário", desc: "Despesas por responsável", icon: <Users className="w-5 h-5" />, color: "bg-blue-100 text-blue-500" },
    { title: "Por Cartão", desc: "Faturas e gastos por cartão", icon: <CreditCard className="w-5 h-5" />, color: "bg-purple-100 text-purple-500" },
    { title: "Por Status", desc: "Pagos vs Pendentes", icon: <CheckCircle2 className="w-5 h-5" />, color: "bg-emerald-100 text-emerald-500" },
    { title: "Fluxo de Caixa", desc: "Receitas vs Despesas", icon: <ArrowRightLeft className="w-5 h-5" />, color: "bg-teal-100 text-teal-500" },
    { title: "Transações Parceladas", desc: "Parcelas pagas e pendentes", icon: <Layers className="w-5 h-5" />, color: "bg-indigo-100 text-indigo-500" },
    { title: "Transações Recorrentes", desc: "Regras e frequência", icon: <RefreshCw className="w-5 h-5" />, color: "bg-pink-100 text-pink-500" },
    { title: "Por Conta Bancária", desc: "Gastos e entradas por conta", icon: <Landmark className="w-5 h-5" />, color: "bg-sky-100 text-sky-500" },
  ];
  ...
  return (
    <div className="flex flex-col min-h-screen bg-transparent transition-colors duration-300 dark:bg-[#0B0F19] font-sans text-slate-900">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-auto flex flex-col items-center">
        ...
        {/* Cabeçalho da Página */}
        <div className="w-full max-w-5xl bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 mb-6 flex flex-col">
          <h1 className="text-xl md:text-2xl font-extrabold text-slate-900 mb-1">Relatórios Financeiros</h1>
          <p className="text-sm text-slate-500">Visualize e exporte relatórios detalhados sobre suas finanças.</p>
        </div>
        ...
        <div className="w-full max-w-5xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {relatorios.map((rel, i) => (
            <div
              ...
              className="bg-white rounded-2xl shadow-sm border border-slate-100 flex flex-col overflow-hidden hover:shadow-md transition-shadow group cursor-pointer"
            >
              <div className="p-6 flex items-start gap-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${rel.color}`}>
                  {rel.icon}
                </div>
                <div className="mt-1">
                  <h3 className="font-bold text-[15px] text-slate-900 leading-tight mb-1">{rel.title}</h3>
                  <p className="text-xs text-slate-500">{rel.desc}</p>
                </div>
              </div>
              
              <div className="px-6 py-4 border-t border-slate-50 mt-auto flex items-center justify-between text-slate-400 group-hover:text-slate-600 transition-colors">
                <span className="text-[13px] font-medium">Ver detalhes</span>
      ...
      {selectedReport && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-[500px] overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">

            {/* Modal Header */}
            <div className="p-8 pb-6 flex items-center gap-4 relative">
              <button
              ...
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${selectedReport.color}`}>
                {selectedReport.icon}
              </div>
              <div>
                <h2 className="text-xl font-extrabold text-slate-900 leading-tight">Transações {selectedReport.title.toLowerCase()}</h2>
                <p className="text-[13px] text-slate-500 mt-1">Selecione o período e formato</p>
              </div>
            </div>
            ...
            <div className="px-8 pb-8 flex flex-col gap-6">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-bold text-slate-500 tracking-wider uppercase">PERÍODO DO RELATÓRIO</span>
              </div>

              <div className="flex flex-col gap-5">
                <div>
                  <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">DATA INICIAL</label>
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">DATA FINAL</label>
                  <div className="flex items-center border border-slate-200 rounded-xl px-4 py-3 bg-white focus-within:ring-2 focus-within:ring-purple-500/20 focus-within:border-purple-500 transition-all">
                    <Calendar className="w-4 h-4 text-slate-400 shrink-0 mr-3" />
                    <input
                      type="text"
              ...
              <div className="flex justify-end gap-3 pt-6 mt-2">
                <button className="px-6 py-2.5 bg-green-50 hover:bg-green-100 text-green-600 font-semibold text-sm rounded-xl transition-colors flex items-center gap-2">
                ...
                <button className="px-6 py-2.5 bg-[#F8F1FF] hover:bg-[#F3E8FF] text-[#9333EA] font-semibold text-sm rounded-xl transition-colors flex items-center gap-2">
