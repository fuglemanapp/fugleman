    ...
    <div className="flex flex-col min-h-screen bg-transparent transition-colors duration-300 dark:bg-[#0B0F19] font-sans text-slate-900">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-auto flex flex-col items-center">
        ...
        <div className="w-full max-w-5xl bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl md:text-2xl font-extrabold text-slate-900 mb-1">Conciliação Bancária</h1>
            <p className="text-sm text-slate-500">Revise e aprove as transações importadas.</p>
          </div>
          <button className="px-6 py-2.5 bg-purple-50 text-purple-300 font-medium text-sm rounded-xl flex items-center justify-center gap-2 cursor-not-allowed">
            <Check className="w-4 h-4" /> Conciliar ( 0 )
          </button>
        </div>

        {/* Toolbar Secundária */}
        <div className="w-full max-w-5xl mb-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3 self-start sm:self-auto ml-2">
            <div className="w-5 h-5 rounded border-2 border-slate-200 bg-white cursor-pointer"></div>
            <span className="text-sm font-medium text-slate-600">Selecionar Tudo</span>
          </div>

          <div className="relative w-full sm:w-auto">
            <select className="w-full sm:w-[220px] appearance-none border border-slate-200 rounded-xl px-4 py-2.5 text-slate-600 font-medium text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all bg-white cursor-pointer flex items-center">
              <option value="todas">Todas Integrações</option>
            </select>
        ...
        <div className="w-full max-w-5xl flex flex-col items-center justify-center py-20 text-center">
          <div className="w-20 h-20 bg-green-50 text-green-500 rounded-full flex items-center justify-center mb-6 shadow-sm border border-green-100">
            <svg className="w-10 h-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5"></path><path d="M20 12v6a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-6"></path></svg>
          </div>
          <h2 className="text-2xl font-extrabold text-slate-900 mb-3">Tudo conciliado!</h2>
          <p className="text-[15px] text-slate-500 max-w-md mx-auto mb-1 leading-relaxed">
            Não há transações pendentes de conciliação no momento.
          </p>
          <p className="text-[15px] text-slate-500 max-w-md mx-auto mb-8 leading-relaxed">
            Você está em dia com suas finanças.
          </p>
          <button className="px-5 py-2.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-800 font-bold text-xs rounded-xl transition-colors shadow-sm flex items-center gap-2">
            <RotateCw className="w-4 h-4" /> Verificar novamente
          </button>
        </div>
