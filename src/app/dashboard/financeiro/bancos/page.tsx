...
export default function BancosPage() {
  ...
  const [modalStep, setModalStep] = useState<'choice' | 'manual'>('choice')
  ...
  const closeModal = () => {
    ...
    setTimeout(() => setModalStep('choice'), 200) // Reset after animation
  }
    ...
    <div className="flex flex-col min-h-screen bg-transparent transition-colors duration-300 dark:bg-[#0B0F19] font-sans text-slate-900">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-auto flex flex-col items-center">
        ...
        <div className="w-full max-w-5xl bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl md:text-2xl font-extrabold text-slate-900 mb-1">Bancos</h1>
            <p className="text-sm text-slate-500">Gerencie suas contas bancárias e saldos.</p>
          </div>
          <button
        ...
        <div className="w-full max-w-5xl flex flex-col items-center justify-center py-20 text-center">
          <div className="relative w-24 h-24 bg-gradient-to-br from-pink-100/50 to-purple-100/50 rounded-full flex items-center justify-center mb-8 shadow-sm">
            <div className="absolute inset-0 bg-white/40 rounded-full backdrop-blur-sm"></div>
            <Landmark className="w-10 h-10 text-pink-500 relative z-10" fill="currentColor" strokeWidth={1.5} />
          </div>
          <h2 className="text-2xl font-extrabold text-slate-900 mb-3 tracking-tight">Nenhuma conta cadastrada</h2>
          <p className="text-[15px] text-slate-500 max-w-[320px] mx-auto mb-8 leading-relaxed">
            Adicione suas contas bancárias para gerenciar seu saldo e transações em um só lugar.
          </p>
          <button
            onClick={() => setIsModalOpen(true)}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-[700px] overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
            ...
            <div className="p-8 pb-6 flex flex-col relative border-b border-slate-100">
              ...
              <div className="flex items-center gap-4 mb-2">
                ...
                <div>
                  <h2 className="text-2xl font-extrabold text-slate-900 leading-tight">Nova Conta</h2>
                  <p className="text-sm text-slate-500 mt-1">Escolha como deseja cadastrar a conta bancária.</p>
                </div>
              </div>
            </div>
            ...
            {modalStep === 'choice' ? (
              <div className="p-8 bg-[#FAFAFA] grid md:grid-cols-2 gap-6">

                {/* Opção Open Finance */}
                <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm hover:border-purple-300 hover:shadow-md transition-all flex flex-col h-full group cursor-pointer">
                  <div className="w-12 h-12 bg-purple-50 text-purple-600 rounded-2xl flex items-center justify-center mb-6">
                    {/* Ícone de Plug */}
                    <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22v-5"></path><path d="M9 8V2"></path><path d="M15 8V2"></path><path d="M18 8v5a4 4 0 0 1-4 4h-4a4 4 0 0 1-4-4V8Z"></path></svg>
                  </div>
                  <h3 className="text-lg font-bold text-slate-900 mb-3">Conta OpenFinance</h3>
                  <p className="text-[13px] text-slate-500 leading-relaxed mb-8 flex-1">
                    Conecte a instituição via Open Finance para importar contas, saldos e transações automaticamente com consentimento seguro.
                  </p>
                  <button className="text-sm font-bold text-purple-600 flex items-center gap-2 group-hover:text-purple-700 transition-colors">
                    Conectar instituição <ArrowRightLeft className="w-4 h-4 ml-1" />
                  </button>
                </div>
                ...
                <div
                  ...
                  className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm hover:border-blue-300 hover:shadow-md transition-all flex flex-col h-full group cursor-pointer"
                >
                  <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center mb-6">
                    <Wallet className="w-6 h-6" />
                  </div>
                  <h3 className="text-lg font-bold text-slate-900 mb-3">Conta Manual</h3>
                  <p className="text-[13px] text-slate-500 leading-relaxed mb-8 flex-1">
                    Cadastre uma conta com saldo inicial e controle lançamentos, ajustes e transferências manualmente. Ideal para contas sem integração.
                  </p>
                  <button className="text-sm font-bold text-blue-600 flex items-center gap-2 group-hover:text-blue-700 transition-colors">
                    Preencher dados da conta <ArrowRightLeft className="w-4 h-4 ml-1" />
                  </button>
                </div>

              </div>
            ) : (
              <div className="p-8 bg-[#FAFAFA] flex flex-col gap-8">
                <div className="grid md:grid-cols-12 gap-8">

                  {/* Esquerda: Dados da Conta */}
                  <div className="md:col-span-7 flex flex-col gap-6">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-blue-500"></div>
                      <span className="text-[10px] font-bold text-slate-500 tracking-wider uppercase">DADOS DA CONTA</span>
                    </div>

                    <div>
                      <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">NOME DA CONTA</label>
                      <input
                        type="text"
                        placeholder="Ex: Nubank, Bradesco..."
                        className="w-full border border-slate-200 rounded-xl px-4 py-3.5 text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-white"
                      />
                    </div>

                    <div>
                      <label className="block text-[11px] font-bold text-slate-700 tracking-wider mb-2 uppercase">SALDO INICIAL (R$)</label>
                      <div className="flex items-center border border-slate-100 rounded-xl px-4 py-3 bg-slate-50 focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-500 transition-all focus-within:bg-white">
                        <span className="text-base font-bold text-slate-400 mr-2">R$</span>
                        <input
                          type="text"
                          placeholder="0,00"
                          className="w-full text-base font-bold text-slate-800 placeholder:text-slate-400 focus:outline-none bg-transparent"
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="md:col-span-5 flex flex-col gap-6">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-[10px] font-bold text-slate-500 tracking-wider uppercase">CONFIGURAÇÕES</span>
                    </div>
                    ...
                    <div className="border border-slate-100 rounded-2xl p-5 flex items-center justify-between bg-white shadow-sm hover:border-slate-200 transition-colors">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-purple-50 text-purple-500 rounded-full flex items-center justify-center shrink-0">
                          {/* Ícone de Estrela com a mesma cor do print */}
                          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>
                        </div>
                        <div>
                          <h4 className="font-bold text-slate-900 text-[13px]">Conta Padrão</h4>
                          <p className="text-[10px] text-slate-400">Usar automaticamente em novos lançamentos.</p>
                        </div>
                      </div>
                      <div
                        onClick={() => setIsMainAccount(!isMainAccount)}
                        className={`w-10 h-5 rounded-full cursor-pointer relative transition-colors duration-300 ${isMainAccount ? 'bg-purple-500' : 'bg-slate-200'}`}
                      >
                        <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-sm transform transition-transform duration-300 ${isMainAccount ? 'translate-x-5.5' : 'translate-x-0.5'}`}></div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex justify-end pt-6 border-t border-slate-100">
                  <button className="px-8 py-3 bg-[#F8F1FF] hover:bg-[#F3E8FF] text-[#9333EA] font-semibold text-sm rounded-xl transition-colors flex items-center gap-2">
                    <Check className="w-4 h-4" /> Salvar Conta
                  </button>
                </div>
              </div>
            )}
