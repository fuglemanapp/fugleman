...
export default function AgendaPage() {
  const [selectedDurations, setSelectedDurations] = useState<string[]>(['1H'])
  ...
  const durationOptions = ['15 MIN', '30 MIN', '45 MIN', '1H', '1H 30M', '2H', '3H']
      ...
      <main className="flex-1 p-4 md:p-8 overflow-auto flex flex-col items-center">
        ...
        <div className="w-full max-w-5xl flex justify-center mb-8">
          <div className="bg-white rounded-full p-1.5 shadow-sm border border-slate-100 flex items-center text-sm font-medium overflow-x-auto max-w-full hide-scrollbar">
            ...
            <a href="/dashboard/conta/agenda" className="px-6 py-2 bg-[#F3E8FF] text-[#9333EA] rounded-full whitespace-nowrap transition-colors">Agenda</a>
          </div>
        </div>
        ...
        <div className="w-full max-w-5xl bg-white rounded-2xl shadow-sm border border-slate-100 p-8 md:p-10 mb-8">
          ...
          <div className="mb-10">
            <h2 className="text-xl font-extrabold text-slate-900 mb-2">Agenda Compartilhada</h2>
            <p className="text-sm text-slate-500">Configure sua agenda pública para agendamentos automáticos.</p>
          </div>

          {/* Link de Agendamento */}
          <div className="mb-10">
            <label className="block text-[11px] font-bold text-slate-500 tracking-wider mb-2">SEU LINK DE AGENDAMENTO</label>
            <div className="flex items-center border border-slate-200 rounded-xl px-4 py-3 bg-white focus-within:ring-2 focus-within:ring-purple-500/20 focus-within:border-purple-500 transition-all">
              <Link2 className="w-4 h-4 text-slate-400 shrink-0 mr-3" />
              <input
                type="text"
                readOnly
                value="https://app.fugleman.com/agenda/compartilhada/GFUAPT2ZgV9VyGcdTqRu6Q"
                className="w-full text-slate-600 font-medium focus:outline-none bg-transparent"
              />
          ...
          {/* Título da Agenda */}
          <div className="mb-10">
            <label className="block text-[11px] font-bold text-slate-500 tracking-wider mb-2">TÍTULO DA AGENDA</label>
            <div className="flex items-center border border-slate-200 rounded-xl px-4 py-3.5 bg-white focus-within:ring-2 focus-within:ring-purple-500/20 focus-within:border-purple-500 transition-all">
              <span className="text-slate-300 font-serif italic font-bold mr-3">T</span>
              <input
                type="text"
                placeholder="Ex: Consultoria Financeira"
                className="w-full text-slate-800 placeholder:text-slate-400 font-medium focus:outline-none bg-transparent"
              />
            </div>
            <p className="text-[10px] text-slate-400 mt-2">Este título será exibido no topo da sua página de agendamento.</p>
          </div>

          {/* Durações Permitidas */}
          <div className="mb-12">
            <label className="block text-[11px] font-bold text-slate-500 tracking-wider mb-3">DURAÇÕES PERMITIDAS</label>
                ...
                return (
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-lg border text-sm font-medium transition-colors ${
                      isActive
                        ? 'border-[#00C853] bg-[#E8F5E9] text-[#00C853] font-bold shadow-sm'
            ...
            <p className="text-[10px] text-slate-400 mt-3">Selecione todas as durações que deseja oferecer aos seus clientes.</p>
          </div>

          {/* Integrações */}
          <div className="grid md:grid-cols-2 gap-8 mb-12">
            <div>
              <label className="block text-[11px] font-bold text-slate-500 tracking-wider mb-2">ADICIONAR EVENTOS AO GOOGLE AGENDA?</label>
              <div className="relative">
                <select className="w-full appearance-none border border-slate-200 rounded-xl px-4 py-3.5 text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all bg-slate-50/50 cursor-pointer pb-3">
                  <option>Não</option>
                  <option>Sim</option>
                </select>
                ...
                <div className="absolute right-10 top-1/2 -translate-y-1/2 flex items-center gap-1.5 pointer-events-none">
                  ...
                  <span className="text-[10px] font-bold text-orange-400 tracking-wide">Integração Google desativada</span>
                </div>
              </div>
              <p className="text-[10px] text-slate-400 mt-2">Define se agendamentos públicos serão enviados ao Google Agenda.</p>
            </div>

            <div>
              <label className="block text-[11px] font-bold text-slate-500 tracking-wider mb-2">GERAR LINK DA REUNIÃO NO GOOGLE MEET?</label>
              <div className="relative">
                <select className="w-full appearance-none border border-slate-200 rounded-xl px-4 py-3.5 text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all bg-slate-50/50 cursor-pointer">
                  <option>Não</option>
                  <option>Sim</option>
                </select>
                <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
              ...
              <p className="text-[10px] text-slate-400 mt-2">Se ativado, o convite terá um link do Google Meet.</p>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="font-bold text-slate-900 text-[15px]">Dias disponíveis</h3>
                <p className="text-[11px] text-slate-500">Defina seus horários de atendimento semanais.</p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 mb-8">
              {weekDays.map((day) => {
                ...
                return (
                  <div key={day} className="flex items-center gap-4 border border-slate-100 rounded-xl p-3 bg-white hover:bg-slate-50/50 transition-colors">
                    <div
                      onClick={() => toggleDay(day)}
                      className={`relative w-10 h-6 rounded-full cursor-pointer shrink-0 transition-colors duration-300 ${
                        isDayActive ? 'bg-[#00C853]' : 'bg-slate-200'
                      }`}
                    >
                      <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow-sm transform transition-transform duration-300 ${
                        isDayActive ? 'translate-x-5' : 'translate-x-1'
                      }`}></div>
                    </div>
                    <span className="font-bold text-slate-900 w-8">{day}</span>
                    <button className="flex items-center gap-1 text-[11px] font-bold text-[#9333EA] hover:text-purple-800 transition-colors ml-4">
                      <span className="text-lg leading-none mb-0.5">+</span> Adicionar intervalo
                    </button>
                  </div>
                )
              })}
