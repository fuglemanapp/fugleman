...
export default function AjudaArtigoConciliacaoOfPage() {
  return (
      ...
      <main className="flex-1 p-4 md:p-8 flex flex-col items-center">
        ...
        <div className="w-full max-w-4xl flex flex-col gap-6">
          ...
          <div className="flex items-center gap-3 theme-text-secondary text-xs font-medium px-2 py-4 mb-2">
            ...
            <span>{'>'}</span>
            <span className="theme-text-primary font-bold">Como funcionam as conciliações com Op...</span>
          </div>
          ...
          <div className="theme-bg-card rounded-[2rem] p-8 md:p-12 shadow-sm border theme-border transition-colors duration-300">
            ...
            <h1 className="text-3xl md:text-4xl font-extrabold theme-text-primary mb-12 tracking-tight">Como funcionam as conciliações com Open Finance?</h1>

            <div className="space-y-8 theme-text-secondary text-[15px] leading-relaxed">
              <p>Como funcionam as conciliações com Open Finance?</p>

              <div>
                <p className="font-bold theme-text-primary text-base mb-1">Resposta rápida:</p>
                <p>O sistema compara automaticamente as transações do banco com os lançamentos já existentes e permite vincular ou ajustar.</p>
              </div>

              <div>
                <p className="mb-2">O que você está vendo na tela</p>
                <p><span className="font-bold theme-text-primary">Esquerda</span>: transações vindas do banco (Open Finance)</p>
                <p><span className="font-bold theme-text-primary">Direita</span>: dados que serão registrados no sistema</p>
                <p><span className="font-bold theme-text-primary">Destaque verde</span>: sugestão de correspondência encontrada</p>
                <p><span className="font-bold theme-text-primary">Cores/labels</span>: indicam status (pago, a pagar, categoria, etc.)</p>
              </div>

              <div>
                <p className="mb-2">Quando aparece "Transação prevista encontrada"</p>
                <p className="mb-1">O sistema encontrou um lançamento parecido (mesmo valor/data/categoria)</p>
                <p className="font-bold theme-text-primary mb-1">Você pode:</p>
                <p><span className="font-bold theme-text-primary">Vincular</span> → une os dois registros</p>
                <p><span className="font-bold theme-text-primary">Ignorar</span> → mantém separado</p>
              </div>

              <div>
                <p className="mb-2">Como conciliar manualmente</p>
                <p>Revise a transação vinda do banco</p>
                <p>Ajuste se necessário:</p>
                <p>Nome/descrição</p>
                <p>Categoria</p>
                <p>Status (pago / a pagar)</p>
                <p>Clique no <span className="font-bold theme-text-primary">check (✔)</span> para confirmar</p>
              </div>

              <div>
                <p className="mb-2">O que cada ação faz</p>
                <p><span className="font-bold theme-text-primary">Check (✔)</span> → confirma e registra a transação</p>
                <p><span className="font-bold theme-text-primary">Vincular</span> → conecta com um lançamento já existente</p>
                <p><span className="font-bold theme-text-primary">Ignorar</span> → não usa a sugestão automática</p>
                <p><span className="font-bold theme-text-primary">Excluir (lixeira)</span> → remove o lançamento</p>
              </div>

              <div>
                <p className="mb-2">Erros comuns (evite)</p>
                <p>Confirmar sem revisar categoria/status</p>
                <p>Vincular transações erradas só porque o valor é igual</p>
                <p>Duplicar lançamentos (não vincular quando já existe)</p>
              </div>

              <div>
                <p className="mb-2">Resultado esperado</p>
                <p>Transações do banco integradas corretamente</p>
                <p>Sem duplicidade</p>
                <p>Dados organizados automaticamente</p>
              </div>

              <div>
                <p className="mb-2">Se algo estiver errado</p>
                <p>Revise se já existe um lançamento igual antes de confirmar</p>
                <p>Verifique categoria e status</p>
                <p>Use "ignorar" se a sugestão estiver incorreta</p>
              </div>

              <div className="pt-4">
                <p>Se persistir: Chame nosso suporte no WhatsApp (47) 99292-1005 ou contato@meuassessor.com</p>
              </div>
            </div>
