...
export function GoogleCalendarIntegration() {
  ...
  return <article className="rounded-3xl border border-[#dcebe2] bg-white p-6 shadow-[0_18px_48px_-34px_rgba(12,100,53,0.42)]"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#edf9f1] text-[#087d3c]"><CalendarDays className="h-5 w-5" /></span><div><h2 className="font-bold">Google Calendar</h2><span className={`text-sm font-semibold ${connection.connected && !needsReconnect ? "text-[#087d3c]" : "text-[#a67020]"}`}>{needsReconnect ? "Permissão adicional necessária" : connection.connected ? "Conectado" : "Aguardando conexão"}</span></div></div><p className="mt-5 leading-6 text-[#648273]">{connection.connected ? "Os novos compromissos serão enviados automaticamente ao calendário principal conectado ao Google. Se os compromissos pararem de aparecer no Google Calendar, use Reconectar." : "Conecte sua conta para importar eventos futuros e enviar os novos compromissos ao Google Calendar."}</p>{connection.error && <p className="mt-4 rounded-2xl bg-[#fff1f1] p-3 text-sm leading-6 text-[#a93636]">{connection.error}</p>}{needsReconnect && <p className="mt-4 rounded-2xl bg-[#fff9ed] p-3 text-sm leading-6 text-[#745c27]">Reconecte para permitir o compartilhamento de leitura da sua agenda familiar no Google Calendar.</p>}<div className="mt-5 flex flex-wrap gap-3">{!connection.connected ? <button onClick={connect} disabled={isConnecting} className="inline-flex items-center gap-2 rounded-xl bg-[#087d3c] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60"><CheckCircle2 className="h-4 w-4" />{isConnecting ? "Abrindo Google…" : "Conectar Google Calendar"}</button> : <><button onClick={() => void synchronize()} disabled={isSyncing} className="inline-flex items-center gap-2 rounded-xl bg-[#087d3c] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60">{isSyncing ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}{isSyncing ? "Sincronizando…" : "Sincronizar agora"}</button><button onClick={connect} disabled={isConnecting} className="inline-flex items-center gap-2 rounded-xl border border-[#b9ddc6] bg-white px-4 py-2.5 text-sm font-bold text-[#087d3c] disabled:opacity-60"><CheckCircle2 className="h-4 w-4" />{isConnecting ? "Abrindo Google…" : "Reconectar Google Calendar"}</button></>}</div>{message && <p className="mt-3 text-sm font-medium text-[#315f48]">{message}</p>}</article>;
}
