    ...
    <header ref={headerRef} className="sticky top-0 z-40 border-b border-[#dcebe2] bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-10">
        <Link href="/dashboard" className="flex shrink-0 items-center" aria-label="Ir para o dashboard do WhatSpent" onClick={dismissMenus}>
          <Image src="/whatspent-logo.svg" alt="WhatSpent" width={184} height={36} className="h-8 w-auto" priority />
        </Link>
            ...
            return (
              <div key={section.label} className="relative">
                <button
                  ...
                  onClick={() => setOpenMenu((current) => reduceOpenMenu(current, { type: "TOGGLE", label: section.label }))}
                  className={`flex items-center gap-1 rounded-xl px-3 py-2 text-sm font-semibold transition-colors ${active ? "bg-[#edf9f1] text-[#087d3c]" : "text-[#5d786a] hover:bg-[#f3f8f5] hover:text-[#17372b]"}`}
                >
                  {section.label}
                ...
                {isOpen && (
                  <div id={menuId} className="absolute left-0 top-[calc(100%+0.6rem)] w-64 rounded-2xl border border-[#dcebe2] bg-white p-2 shadow-[0_24px_48px_-28px_rgba(12,100,53,0.36)]">
                    {section.links.map((link) => (
                      <Link key={link.href} href={link.href} onClick={dismissMenus} className={`block rounded-xl px-3 py-2.5 transition-colors ${link.href === activePath ? "bg-[#edf9f1]" : "hover:bg-[#f5faf6]"}`}>
                        <span className="block text-sm font-semibold text-[#214235]">{link.label}</span>
                        <span className="mt-0.5 block text-xs text-[#789083]">{link.description}</span>
                      </Link>
                    ))}
                  </div>
                )}
        ...
        <div className="hidden items-center gap-3 sm:flex">
          <Link href="/dashboard/ajuda" onClick={dismissMenus} className="grid h-9 w-9 place-items-center rounded-xl text-[#638072] transition-colors hover:bg-[#edf8f1] hover:text-[#087d3c]" aria-label="Ajuda"><CircleHelp className="h-5 w-5" /></Link>
          <Link href="/dashboard/conta" onClick={dismissMenus} className="rounded-xl bg-[#edf9f1] px-3 py-2 text-sm font-semibold text-[#087d3c] transition-colors hover:bg-[#dff5e7]">Minha conta</Link>
        </div>

        <div className="relative lg:hidden">
          <button type="button" onClick={() => setIsMobileMenuOpen((open) => !open)} className="grid h-10 w-10 place-items-center rounded-xl border border-[#dcebe2] text-[#315f48] hover:bg-[#f1faf4]" aria-label={isMobileMenuOpen ? "Fechar menu" : "Abrir menu"} aria-expanded={isMobileMenuOpen} aria-controls="mobile-navigation">
          ...
          {isMobileMenuOpen && (
            <nav id="mobile-navigation" className="absolute right-0 top-[calc(100%+0.6rem)] max-h-[75vh] w-[min(20rem,calc(100vw-2rem))] overflow-y-auto rounded-2xl border border-[#dcebe2] bg-white p-3 shadow-[0_24px_48px_-28px_rgba(12,100,53,0.36)]" aria-label="Menu móvel">
              {sections.map((section) => (
                <div key={section.label} className="border-b border-[#edf3ef] py-3 last:border-0">
                  <p className="px-2 text-xs font-bold uppercase tracking-[0.1em] text-[#8ca297]">{section.label}</p>
                  {section.links.map((link) => (
                    <Link key={link.href} href={link.href} onClick={dismissMenus} className={`mt-1 block rounded-xl px-2 py-2 text-sm font-semibold ${link.href === activePath ? "bg-[#edf9f1] text-[#087d3c]" : "text-[#315f48] hover:bg-[#f5faf6]"}`}>{link.label}</Link>
                  ))}
                </div>
              ))}
              <div className="flex gap-2 pt-3">
                <Link href="/dashboard/conta" onClick={dismissMenus} className="rounded-xl bg-[#edf9f1] px-3 py-2 text-sm font-semibold text-[#087d3c]">Minha conta</Link>
                <Link href="/dashboard/ajuda" onClick={dismissMenus} className="rounded-xl px-3 py-2 text-sm font-semibold text-[#638072] hover:bg-[#f3f8f5]">Ajuda</Link>
              </div>
            </nav>
          )}
        </div>
