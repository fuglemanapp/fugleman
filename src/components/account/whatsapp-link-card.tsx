...
export function WhatsAppLinkCard({ initialPhone }: WhatsAppLinkCardProps) {
  ...
  async function savePhone(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.error || "Não foi possível vincular seu WhatsApp.");
  ...
  return <section className="rounded-[1.75rem] border border-[#dcebe2] bg-white p-6 shadow-[0_20px_50px_-40px_rgba(12,100,53,.36)] sm:p-7">
    ...
    </div> : <form onSubmit={savePhone} className="mt-6">
    ...
    {error && <p role="alert" className="mt-4 rounded-xl bg-[#fff0ed] px-3 py-2.5 text-sm text-[#a1453f]">{error}</p>}
  </section>;
}
